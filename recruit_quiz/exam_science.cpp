...
QuestionList CreatePhysicsExam()
{
	...
	{	//等速直線運動
		...
		questions.push_back({
			"ある車が" + to_string(v * t / 60) + "km離れた地点まで移動するのに" + 
	...
	{	//重力加速度
		...
		int x = 98 * t * t / 2;	//１０倍にすることで、小数点以下第１位までを整数として表す
		...
		questions.push_back({
			...
			to_string(t) + "秒間でXm落下する。\n" + "Xの値を小数点以下第１位まで求めよ(空気抵抗はないものとする)。",
			answer });
	...
	{	//浮力
		...
		questions.push_back({
			...
			to_string(x) + "gを示した\n。" + "この物体を完全に水に入れたところ、はねはかりは"
			+ to_string(y) + "gを示した。\nこのとき、物体にはたらく浮力はXニュートンである。\n" +
			"Xの値を小数点以下第２位を四捨五入して求めよ。", answer });
